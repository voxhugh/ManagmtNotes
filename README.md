...
1. 执行计划分析：用 `EXPLAIN` 查 `type`、`key`、`rows` 定位瓶颈。
2. 索引优化：建复合索引遵最左原则，用覆盖索引免回表。
3. 子查询优化：复杂子查询转 `JOIN`，大结果集用 `EXISTS` 替 `IN`。
4. 聚合查询优化：`GROUP BY` 无 `ORDER BY` 加 `ORDER BY NULL`，先 `WHERE` 过滤再聚合。
5. 架构优化：大表分区，适度反规范化减连接并控维护成本。
6. 连接顺序：多表连接选少记录驱动表减小结果集。
7. 索引列运算：索引列勿用函数或表达式，防索引失效。
...
集合查询：按行操作，含 UNION（并）、INTERSECT（交）、EXCEPT（差）运算，要求结果集元组结构相同 。 
