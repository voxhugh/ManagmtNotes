...
add_executable(MyExe main.cpp)             # 指定目标文件

add_library(MyLib STATIC library.cpp)      # 创建库目标

target_link_libraries(MyExe MyLib)         # 链接目标文件和库

find_package(Boost 1.70 REQUIRED)          # 查找库，指定版本

include_directories(MyPrj/include)         # 设置头文件目录

link_directories(${Boost_LIBRARY_DIRS})    # 设置链接目录

target_include_directories(MyExe PRIVATE ${PROJECT_SOURCE_DIR}/include)  # 指定库的头文件
...
$ b 13                          # 第13行设断点
$ b 20 if i==5                  # 条件断点
$ ignore 1 100                  # 忽略前100次命中
$ commands 1 / end              # 命中时执行脚本
$ i b                           # 查看所有断点
$ d 1-2                         # 删除断点
$ dis 3 4                       # 禁用断点
$ ena 3                         # 启用断点
...
$ p ival                        # 打印变量值
$ pt ival                       # 打印变量类型
$ x/8xg 0x7fff100               # 查看内存（16进制）
$ p/x *(void**)p                # 访存
$ disas                         # 反汇编
$ set disassembly-flavor intel  # 切换Intel汇编语法
$ display a[i]                  # 自动显示变量
$ i display                     # 查看自动显示列表
$ undisplay 2-4                 # 删除自动显示
$ dis display 1                 # 禁用自动显示
...
- **.text段**：程序的机器指令，只读
- **.data段**：已初始化的全局与静态变量，构建后持久化
- **.bss段**：未初始化的全局与静态变量，系统零初始化
...
- LT模式: 默认模式，会持续通知事件；
- ET模式: 搭配EPOLLET标志和非阻塞IO，仅在状态变化时触发一次，需循环读写至EAGAIN，效率更高。
...
- **协议特性**：无连接、不可靠的报文传输层协议，默认阻塞通信。
- **通信模式**：无需建立连接，每次通信需指定目标地址；两端对等，无严格客户端/服务端区分。
- **端口绑定**：接收端需固定绑定端口，非接收端由系统自动分配随机端口。

---

**广播**

- **定义**：通过广播地址向局域网内所有主机发送单点数据的一对多通信方式。
- **实现**：使用子网主机位全1的特殊IP（如 `192.168.1.255`）。
- **特点**：
  - 单向“1:N”通信，子网内所有主机强制接收。
  - 仅限局域网，广域网不转发。
  - 低延迟、低开销，但无法主动拒绝接收。
  - 接收端需绑定端口，终止进程方可停止接收。
...
- **定义**：通过组播组地址实现单点对多主机的灵活一对多通信。
- **分类**：
  - *局部链路*：`224.0.0.0~224.0.0.255`（仅限局域网）。
  - *全局可用*：`224.0.1.0~238.255.255.255`（支持广域网）。
  - *本地管理*：`239.0.0.0~239.255.255.255`（局部可控范围）。
- **特点**：
  - 支持单源/多源发送，接收端需主动加入组播组。
  - 路由器可转发，跨网络适用。
  - 灵活性优于广播，需手动启用组播功能（类似“订阅-发布”模式）。
